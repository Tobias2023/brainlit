...
def napari_viewer(img, labels=None, shapes=None, points=None, label_name="Segmentation"):
    with napari.gui_qt():
        ...
        if shapes is not None:
            if isinstance(labels, list):
                for l,shape in enumerate(shapes):
                    name = label_name + "_" + str(l) 
                    viewer.add_shapes(
                        data=shape, shape_type="path", edge_color="blue"
                    )
            else:
                viewer.add_shapes(
